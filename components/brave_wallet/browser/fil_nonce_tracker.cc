...
uint256_t GetHighestLocallyConfirmed(
  ...
  for (auto& meta : metas) {
    ...
    highest = std::max(highest, fil_meta->tx()->nonce().value() + (uint256_t)1);
  }
  return highest;
}
...
void FilNonceTracker::GetNextNonce(const FilAddress& from,
  ...
  const std::string address = from.ToString();
...
void FilNonceTracker::OnGetNetworkNonce(FilAddress from,
  ...
  auto confirmed_transactions = tx_state_manager_->GetTransactionsByStatus(
      mojom::TransactionStatus::Confirmed, from.ToString());
  ...
  auto pending_transactions = tx_state_manager_->GetTransactionsByStatus(
      mojom::TransactionStatus::Submitted, from.ToString());
