...
class ExternalWalletHandler {
 public:
  ...
  virtual std::string GetAuthorizationURL() = 0;

  virtual Future<absl::optional<ExternalWallet>> HandleAuthorizationResponse(
      const base::flat_map<std::string, std::string>& auth_params) = 0;

  // TODO(zenparsing): Should the result include error details?
